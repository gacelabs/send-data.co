...
class GeneralHelper{
	...
	public static function publishedPagesOnly($DataObject=false)
	{
		if (ClassInfo::exists($DataObject)) {
			// debug::endshow($DataObject);
			$ArrayList = ArrayList::create();
			foreach ($DataObject as $key => $Object) {
				if (ClassInfo::hasMethod($Object, 'isPublished') AND $Object->isPublished()) {
					$ArrayList->push($Object);
				}
			}
			// debug::endshow($ArrayList);
			if ($ArrayList->Count()) {
				return $ArrayList;
			}
		}
	...
	public static function publishedDataOnly($DataObject=false, $Function=false)
	{
		$ArrayList = ArrayList::create();
		if (ClassInfo::exists($DataObject)) {
			// debug::endshow($DataObject);
			foreach ($DataObject as $key => $Object) {
				...
				if (ClassInfo::hasMethod($Object, $Function)) {
					if (ClassInfo::hasMethod($Object->{$Function}(), 'isPublished')) {
						$IsParentPublished = $Object->{$Function}()->isPublished();
					}
				}
				if ((isset($Object->Published) AND $Object->Published == 1) AND $IsParentPublished) {
					$ArrayList->push($Object);
				}
			}
		}
